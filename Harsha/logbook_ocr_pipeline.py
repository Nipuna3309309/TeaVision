...
def fuzzy_best_match(ocr_text, vocabulary, min_score=0.40):
    ...
    scores = sorted([(c, fuzzy_score(t,c.lower())) for c in vocabulary], key=lambda x:-x[1])
...
def make_ocr_engine():
    for kwargs in [
        ...
        try:
            ...
            print(f'OCR loaded {list(kwargs.keys())}')
            return eng
        except Exception:
            continue
...
def run_ocr(img_bgr):
    ...
    try:
        if hasattr(OCR_ENGINE, 'predict'):
            try:
                out = OCR_ENGINE.predict(img_bgr, use_doc_orientation_classify=False,
...
def split_left_right(bgr, pad=3):
    ...
    vk=max(30,h//8)
    vm=cv2.morphologyEx(bi,cv2.MORPH_OPEN,cv2.getStructuringElement(cv2.MORPH_RECT,(1,vk)))
    proj=np.convolve(np.sum(vm[int(h*0.2):],axis=0).astype(float),np.ones(max(15,w//200)),mode='same')
    ...
    tables=[(i+1,bgr[:,max(0,x1-pad):min(w,x2+pad)].copy())
            for i,(x1,x2) in enumerate([(0,mid),(mid,w)]) if x2-x1>w*0.1]
...
def crop_cell(bgr, hlines, vlines, row, col):
    ...
    crop=bgr[y1:y2,x1:x2].copy(); p=CELL_PAD_PX
    return cv2.copyMakeBorder(crop,p,p,p,p,cv2.BORDER_CONSTANT,value=(255,255,255))

def ocr_single_cell(bgr_crop, colidx):
    if bgr_crop is None or bgr_crop.size==0: return '',0.0
    gray=cv2.cvtColor(bgr_crop,cv2.COLOR_BGR2GRAY)
    if colidx in NARROW_COLS: gray=scale_up_narrow(gray)
    else:                     gray=scale_up(gray)
    if colidx in NARROW_COLS:
        ...
        variants=[prep_numeric(gray),prep_clahe_only(gray),v3]
    else:
        cg=cv2.createCLAHE(2.0,(4,4)).apply(gray)
        _,v3=cv2.threshold(cg,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        if np.mean(v3)>127: v3=cv2.bitwise_not(v3)
        variants=[prep_text(gray),prep_clahe_only(gray),v3]
...
def extract_all_cells(bgr, hlines, vlines):
    ...
    results={}; filtered_bleed=0; t0=time.time()
    for r in range(nrows):
        for c in range(ncols):
            crop=crop_cell(bgr,hlines,vlines,r,c)
            text,conf=ocr_single_cell(crop,c)
            if text: results[(r,c)]={'text':text,'conf':conf}
            elif conf==0.0: filtered_bleed+=1
        if (r+1)%10==0: print(f'  Row {r+1}/{nrows} | {time.time()-t0:.0f}s')
    print(f'  Done {len(results)}/{nrows*ncols} bleed={filtered_bleed} {time.time()-t0:.1f}s')
...
def detect_data_start(raw_results, nrows, max_scan=15, fallback=5):
    ...
    for r in range(min(nrows,max_scan)):
        ...
        has_no=bool(NO_PAT.match(t0)); has_month=bool(MONTH_PAT.match(t1))
        n_filled=sum(1 for c in range(17) if raw_results.get((r,c),{}).get('text','').strip())
...
def find_data_extent(cells, ncols, max_empty_streak=8):
    ...
    for r in range(max_r+1):
        ...
        has_month=bool(re.fullmatch(r"[A-Za-z]{1,4}|PR|PRN|TOTAL",month_v))
...
def trim_data_region(cells, hlines, vlines, min_row_fill=0.05, min_col_fill=0.05):
    ...
    new_cells={(r-r0,c-c0):v for (r,c),v in cells.items() if r0<=r<=r1 and c0<=c<=c1}
...
def write_filled_template(cells_by_table, conf_by_table, col_widths_by_table, row_hts_by_table, template_path, out_path):
    ...
    for tidx,cells in cells_by_table.items():
        ...
        try: tmpl_wb=load_workbook(template_path); ws_src=tmpl_wb.active
        except Exception: tmpl_wb=Workbook(); ws_src=tmpl_wb.active
...
def main():
    ...
    for img_path in images:
        base       = img_path.stem
        out_excel  = str(OUTPUT_FOLDER / f'{base}_filled.xlsx')
        print('='*62)
        print(f'  Input : {img_path}')
        print(f'  Output: {out_excel}')
        print('='*62)
        bgr = cv2.imread(str(img_path))
        if bgr is None: print(f'Cannot read {img_path}, skipping.'); continue
        print(f'[1] Image: {bgr.shape[1]}x{bgr.shape[0]}px')
        bgr = rectify_document(bgr)
        print(f'    Rectified: {bgr.shape[1]}x{bgr.shape[0]}px')
        layout, tables = auto_split_tables(bgr)
        print(f'[2] Layout: {layout} | Tables: {len(tables)}')
        all_cells={}; all_confs={}; col_widths_px={}; row_hts_px={}
        for tidx, tbgr in tables:
            label = 'Left' if tidx==1 else ('Right' if tidx==2 else f'Table{tidx}')
            print(f'[3] Table {tidx} ({label}) - {tbgr.shape[1]}x{tbgr.shape[0]}px')
            ...
            ncd  = max((c for r,c in cleaned), default=0)+1
            ...
            cleaned, hlines, vlines = trim_data_region(cleaned, hlines, vlines,
                                                         min_row_fill=0.05, min_col_fill=0.05)
            nrows=len(hlines)-1; ncols=len(vlines)-1
            print(f'    Final {nrows}x{ncols} | {len(cleaned)} cells')
            col_widths_px[tidx] = [int(vlines[i+1]-vlines[i]) for i in range(ncols)]
            row_hts_px[tidx]    = [int(hlines[i+1]-hlines[i]) for i in range(nrows)]
            all_cells[tidx]  = cleaned
            all_confs[tidx]  = conf_dict
        print('[4] Writing Excel...')
        write_filled_template(all_cells, all_confs, col_widths_px, row_hts_px, TEMPLATE_PATH, out_excel)
